    ...
    <script type="text/javascript">
        (function($) {
            ...
            $(document).ready(function() {
                if (travelLoad == 0) {
                    @include('admin.settings.variables_method.JS_DT.travel_method_js')
                        travelLoad = 1;
                }
            });

            $('[data-table="payment"]').one('click', function (e) {
                @include('admin.settings.variables_method.JS_DT.payment_method_js')
            });

            $('[data-table="employee_qualification"]').one('click', function (e) {
                @include('admin.settings.variables_method.JS_DT.employee_qualification_js')
            });

            $('[data-table="languageSkills"]').one('click', function (e) {
                @include('admin.settings.variables_method.JS_DT.language_skills_js')
            });

            $('[data-table="otherSkills"]').one('click', function (e) {
                @include('admin.settings.variables_method.JS_DT.general_skills_js')
            });

            $('[data-table="job_category"]').one('click', function (e) {
                @include('admin.settings.variables_method.JS_DT.job_category_js')
            });
        })(jQuery);
    </script>
@endpush
