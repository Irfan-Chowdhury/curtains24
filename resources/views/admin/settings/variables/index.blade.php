...
<script type="text/javascript">
    (function($) {
        ...
        $(document).ready(function() {
            if (leaveLoad == 0) {
                @include('admin.settings.variables.JS_DT.leave_type_js')
                    leaveLoad = 1;
            }
        });
        ...
        $('[data-table="award"]').one('click', function (e) {
            @include('admin.settings.variables.JS_DT.award_type_js')
        });

        $('[data-table="warning"]').one('click', function (e) {
            @include('admin.settings.variables.JS_DT.warning_type_js')
        });

        $('[data-table="termination"]').one('click', function (e) {
            @include('admin.settings.variables.JS_DT.termination_type_js')
        });

        $('[data-table="expense"]').one('click', function (e) {
            @include('admin.settings.variables.JS_DT.expense_type_js')
        });

        $('[data-table="status"]').one('click', function (e) {
            @include('admin.settings.variables.JS_DT.status_type_js')
        });

        $('[data-table="document"]').on('click', function (e) {
            @include('admin.settings.variables.JS_DT.document_type_js')
        });

        $('[data-table="company_type"]').on('click', function (e) {
            @include('admin.settings.variables.JS_DT.company_type_js')
        });

        $('[data-table="relation_type"]').on('click', function (e) {
            @include('admin.settings.variables.JS_DT.relation_type_js')
        });

        $('[data-table="loan_type"]').on('click', function (e) {
            @include('admin.settings.variables.JS_DT.loan_type_js')
        });

        $('[data-table="deduction_type"]').on('click', function (e) {
            @include('admin.settings.variables.JS_DT.deduction_type_js')
        });

        $('[data-table="deposit_category"]').on('click', function (e) {
            @include('admin.settings.variables.JS_DT.deposit_category_js')
        });

        $('[data-table="job_experience"]').on('click', function (e) {
            @include('admin.settings.variables.JS_DT.job_experience_js')
        });
